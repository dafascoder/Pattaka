...
} from "@/components/builder/nodes/helpers";
import type {
	Agent,
	Node,
	PaletteItem,
} from "@/components/builder/nodes/types";
	...
	devtools(
		(set, get) => ({
			...
			addNode: (paletteItem: PaletteItem, position: { x: number; y: number }) =>
				set(
					(state) => {
						// Prevent adding multiple triggers
						if (
							paletteItem.category === "triggers" &&
							state.workflow.hasTrigger
						) {
							// Trigger error event for user feedback
						...
						const newHasTrigger =
							state.workflow.hasTrigger || paletteItem.category === "triggers";
			...
			removeNode: (nodeId: string) =>
				set(
					(state) => {
						const nodeToRemove = state.workflow.nodes.find(
							(n) => n.id === nodeId
						);
						...
						const newHasTrigger =
							nodeToRemove?.data?.nodeType === "trigger"
				...
				set(
					(state) => ({
						workflow: {
							...
							...(info.description && {
								workflowDescription: info.description,
							}),
							isModified: true,
						},
					}),
				...
				set(
					(state) => ({
						agents: {
							...state.agents,
							agents,
							agentPaletteItems: agents.map((agent) =>
								createAgentPaletteItem(agent)
							),
							loadingAgents: false,
							agentsError: null,
						},
					...
					(state) => {
						...
						return {
							agents: {
								...state.agents,
								agents: newAgents,
								agentPaletteItems: newAgents.map((a) =>
									createAgentPaletteItem(a)
								),
							},
						};
					},
			...
			removeAgent: (agentId: string) =>
				set(
					(state) => {
						const newAgents = state.agents.agents.filter(
							(a) => a.id !== agentId
						);
						return {
							agents: {
				...
				set(
					(state) => ({
						workflow: {
							...
							hasTrigger: (workflowData.nodes || []).some(
								(n: any) => n.data?.nodeType === "trigger"
							),
							isModified: false,
						},
						ui: {
...
export const useSearchTerm = () =>
	useWorkflowBuilderStore((state) => state.ui.searchTerm);
export const useDragState = () =>
	useWorkflowBuilderStore((state) => state.drag);
export const useWorkflowNodes = () =>
	useWorkflowBuilderStore((state) => state.workflow.nodes);
export const useWorkflowEdges = () =>
	useWorkflowBuilderStore((state) => state.workflow.edges);
export const useHasTrigger = () =>
	useWorkflowBuilderStore((state) => state.workflow.hasTrigger);
export const useAgents = () =>
	useWorkflowBuilderStore((state) => state.agents.agents);
export const useAgentPaletteItems = () =>
	useWorkflowBuilderStore((state) => state.agents.agentPaletteItems);
export const useSelectedNodeId = () =>
	useWorkflowBuilderStore((state) => state.ui.selectedNodeId);
export const useSidebarExpanded = () =>
	useWorkflowBuilderStore((state) => state.ui.sidebarExpanded);
