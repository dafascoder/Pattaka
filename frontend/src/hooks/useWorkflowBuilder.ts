...
export function useWorkflowBuilder(options: UseWorkflowBuilderOptions = {}) {
  ...
  const exportWorkflow = useWorkflowBuilderStore(state => state.exportWorkflow)
  const importWorkflow = useWorkflowBuilderStore(state => state.importWorkflow)
  ...
  const workflowStats = {
    ...
    triggerCount: workflow.nodes.filter(n => n.data.nodeType === 'trigger').length,
    actionCount: workflow.nodes.filter(n => n.data.nodeType === 'action').length,
    agentCount: workflow.nodes.filter(n => n.data.nodeType === 'agent').length,
  }
