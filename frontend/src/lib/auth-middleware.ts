...
const fetchUser = createServerFn({ method: "GET" }).handler(async () => {
	...
	try {
		...
		if (!cookieHeader) {
			authLogger.info(
				"Auth middleware - No cookies available, skipping API call"
			);
			return null;
		}
		...
		authLogger.info(
			"Auth middleware - Go backend response status:",
			response.status
		);
		...
		if (response.ok) {
			...
			if (json.success && json.data) {
				...
				authLogger.info(
					"Auth middleware - User authenticated successfully (cached)",
					{
				...
				authLogger.warn(
					"Auth middleware - Go backend returned unsuccessful response",
					json
				);
		...
		authLogger.error(
			"Auth middleware - Error checking authentication with Go backend:",
			err
		);
		// Clear cache on error
		authCache = null;
...
export const authMiddleware = createMiddleware({
	...
	authLogger.info(
		"Auth middleware - User context set, proceeding to next middleware"
	);
	return next({
		context: {
			user,
...
export const testAuthMiddleware = createServerFn({ method: "GET" }).handler(
	async () => {
		const user = await fetchUser();
		return {
			authenticated: !!user,
